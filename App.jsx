...
const TECHNICIANS = ["Francisco José Hernández", "Amador García", "Pedro Jesús Jiménez", "Antonio Raúl Fernández", "Mauricio Giovanni Coronel"];
...
const getAlbaranRef = (orden) => {
  const yr = new Date().getFullYear().toString().slice(-2);
  const num = orden.trim().replace(/\D/g, "");
  return num ? `${yr}alb-0${num}` : null;
};
...
async function buildPDF(data) {
  ...
  const albaranRef = getAlbaranRef(data.s1.orden);
...
function ReportScreen({ data, onReset }) {
  ...
  const sections = [
    { title: "Datos del Servicio", icon: "📋", rows: [
      ...
      ["Referencia Albarán", albaranRef ? <span style={{ color: BRAND_RED }} className="font-black">{albaranRef}</span> : null],
      ...
      {albaranRef && (
        <div className="rounded-2xl p-4" style={{ background: "rgba(14,116,144,0.08)", border: "1px solid rgba(14,116,144,0.25)" }}>
          ...
          <p className="text-slate-400 text-xs leading-relaxed">
            Abre el albarán <span className="text-sky-300 font-bold">{albaranRef}</span> en Stel Order y adjunta:
          </p>
...
export default function App() {
  ...
  const [s1, setS1] = useState({ tecnico:"", orden:"", fecha:today, tipo:"" });
  ...
  const reset = () => {
    ...
    setS1({tecnico:"",orden:"",fecha:today,tipo:""});
  ...
  const albaranPreview = getAlbaranRef(s1.orden);
            ...
            <div className="flex flex-col gap-4 pb-4">
              ...
              {step===1 && (
                <div className="flex flex-col gap-3">
                  ...
                  <div className="rounded-2xl p-4 flex flex-col gap-2" style={cardStyle}>
                    <label className="text-white font-semibold text-sm flex items-center gap-2">
                      Nº de Albarán (Stel Order) <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full tracking-widest uppercase" style={badgeRed}>Obligatorio</span>
                    </label>
